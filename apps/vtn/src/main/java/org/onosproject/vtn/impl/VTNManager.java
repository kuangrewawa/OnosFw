...
public class VTNManager implements VTNService {
    ...
    public void onServerDetected(Device device) {
        ...
        log.info("handler---------------------------" + (null == handler));
        TunnelConfig config = handler.behaviour(TunnelConfig.class);
        BridgeConfig bridgeConfig = handler.behaviour(BridgeConfig.class);
        log.info("TunnelConfig-----------------" + config);
        log.info("bridgeConfig-----------------" + bridgeConfig);
    ...
    private class InnerDeviceListener implements DeviceListener {
        ...
        public void event(DeviceEvent event) {
            ...
            log.info("Device Type {}: DeviceEvent Type {}", device.type(),
                     event.type());
            log.info("Device Type {}: DeviceEvent Type {}",
                     Device.Type.CONTROLLER == device.type(),
                     DeviceEvent.Type.DEVICE_ADDED == event.type());
            if (Device.Type.CONTROLLER == device.type()
                    && DeviceEvent.Type.DEVICE_ADDED == event.type()) {
                log.info("---------------controller----node detected add------------------");
                backgroundService.execute(() -> {
                    ...
                    && DeviceEvent.Type.DEVICE_AVAILABILITY_CHANGED == event
                            .type()) {
                log.info("-----------------controller--node detected remove------------------");
                backgroundService.execute(() -> {
                    onServerVanished(device);
                });
            } else if (Device.Type.SWITCH == device.type()
                    && DeviceEvent.Type.DEVICE_ADDED == event.type()) {
                log.info("--------------switch-----node detected add------------------");
                backgroundService.execute(() -> {
                    ...
                    && DeviceEvent.Type.DEVICE_AVAILABILITY_CHANGED == event
                            .type()) {
                log.info("--------------switch-----node dprogramLocalOutetected remove------------------");
                backgroundService.execute(() -> {
