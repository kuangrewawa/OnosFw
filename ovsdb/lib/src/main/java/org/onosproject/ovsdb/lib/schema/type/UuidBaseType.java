...
import org.onosproject.ovsdb.lib.notation.RefTableRow;
 ...
 */
public class UuidBaseType implements BaseType {
    /**
     * RefType is strong or weak. refer to <base-type> RFC 7047 Section 3.2.
     */
    public enum RefType {
    ...
    public UuidBaseType() {
        this.refTable = null;
        this.refType = RefType.STRONG.getName();
    }
        ...
        if (value.isArray()) {
            if (value.size() == 2) {
                if (value.get(0).isTextual()
                        ...
                        || "named-uuid".equals(value.get(0).asText())) {
                    return UUID.uuid(value.get(1).asText());
                }
            }
        } else {
            return new RefTableRow(refTable, value);
        }
        return null;
    }
