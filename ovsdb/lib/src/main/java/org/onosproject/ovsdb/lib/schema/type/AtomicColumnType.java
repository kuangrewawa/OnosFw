 ...
 */
public class AtomicColumnType implements ColumnType {
    ...
    /**
     * If "min" or "max" is not specified, If "min" is not 1 or "max" is not 1,
     * or both, and "value" is not specified, the type is a set of scalar type
     * "key". Refer to RFC 7047, Section 3.2 <type>.
     */
                ...
                if (json.size() == 2) {
                    if (json.get(0).isTextual()
                        ...
                        for (JsonNode node : json.get(1)) {
                            set.add(baseType.toValue(node));
                        }
                    } else {
                        set.add(baseType.toValue(json));
                    }
                }
            } else {
                set.add(baseType.toValue(json));
